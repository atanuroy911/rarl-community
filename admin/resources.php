...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && adminCsrfOk()) {
    ...
    if ($action === 'add_category') {
        ...
        if ($name) {
            $pdo->prepare("INSERT INTO resource_categories (name,slug,description,icon_emoji,is_published) VALUES (?,?,?,?,?)")
                ->execute([$name, trim($slug, '-'), $desc, $icon, $pub]);
    ...
    if ($action === 'add_resource') {
        ...
        if ($title && $url && $catId) {
            $pdo->prepare("INSERT INTO resources (category_id,title,description,url,type,is_featured) VALUES (?,?,?,?,?,?)")
                ->execute([$catId, $title, $desc, $url, $type, $feat]);
    ...
    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
      ...
      <div class="mt-5 space-y-2">
        ...
        <div class="flex items-center justify-between p-2 bg-gray-50 border border-gray-100 rounded-lg group">
          <div class="flex items-center gap-2"><span class="text-base"><?= $c['icon_emoji'] ?></span><span class="text-xs font-semibold text-gray-700"><?= htmlspecialchars($c['name']) ?></span></div>
          <form method="POST" class="opacity-0 group-hover:opacity-100 transition-opacity" onsubmit="return confirm('Delete category? This will not delete its resources, but they will be orphaned.')">
