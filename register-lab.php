...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!array_key_exists($vals['years_experience'], $yearsOptions)) $errors[] = 'Please select the lab\'s years of research experience.';
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-950 py-12 px-4">
  <div class="max-w-xl mx-auto">
    ...
    <form method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-7 shadow-sm space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" name="country" value="<?= htmlspecialchars($vals['country'] ?? '') ?>" required
            placeholder="e.g. USA" class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rarl-red/25 focus:border-rarl-red transition-all"/>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Google Scholar URL <span class="text-rarl-red">*</span></label>
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">ORCID ID <span class="text-rarl-red">*</span></label>
