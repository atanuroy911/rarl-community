...
<section class="relative overflow-hidden" style="background:linear-gradient(135deg,<?= BRAND_INK ?> 0%,<?= BRAND_INK_SOFT ?> 100%);">
  <div class="relative z-10 max-w-5xl mx-auto px-6 py-16 text-center">
    <div class="text-5xl mb-4"><i class="fa-solid fa-people-group"></i></div>
...
<div class="max-w-6xl mx-auto px-6 py-14">
  ...
  <div class="bg-white dark:bg-gray-900 border border-amber-200 dark:border-amber-800 rounded-2xl p-14 text-center max-w-lg mx-auto shadow-sm">
    <div class="text-5xl mb-4"><i class="fa-solid fa-lock"></i></div>
    ...
    <?php if ($isMember): ?>
    <p class="text-amber-600 text-sm font-semibold"><i class="fa-solid fa-hourglass-half"></i> Your account is pending approval — you'll get access as soon as you're approved.</p>
    <?php else: ?>
  ...
  <form method="GET" class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm mb-8 flex flex-wrap gap-3 items-center">
    ...
    <label class="flex items-center gap-2 px-3 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl text-sm cursor-pointer">
      <input type="checkbox" name="mentors" value="1" <?= !empty($_GET['mentors']) ? 'checked' : '' ?> class="accent-rarl-red w-4 h-4"/> <i class="fa-solid fa-compass"></i> Mentors only
    </label>
  ...
  <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-14 text-center text-gray-400">
    <p class="text-4xl mb-3"><i class="fa-solid fa-magnifying-glass"></i></p>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm hover:-translate-y-0.5 hover:shadow-lg transition-all">
      ...
      <div class="flex items-center gap-1.5 flex-wrap mb-3">
        <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full <?= $m['type']==='lab'?'bg-blue-50 text-blue-600':'bg-purple-50 text-purple-600' ?>"><?= $m['type']==='lab'?'<i class="fa-solid fa-building-columns"></i> Lab':'<i class="fa-solid fa-user-graduate"></i> Individual' ?></span>
        ...
        <?php if (!empty($m['open_to_mentor'])): ?><span class="text-[10px] font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-full"><i class="fa-solid fa-compass"></i> Mentor</span><?php endif; ?>
        <?php if (!empty($m['seeking_mentor'])): ?><span class="text-[10px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full"><i class="fa-solid fa-bullseye"></i> Seeking Mentor</span><?php endif; ?>
      </div>
      <?php if ($interests): ?>
      ...
      <div class="flex items-center gap-2">
        ...
        <a href="<?= htmlspecialchars($m['google_scholar_url']) ?>" target="_blank" rel="noopener" title="Google Scholar" class="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-rarl-red hover:text-white text-gray-500 dark:text-gray-300 flex items-center justify-center text-xs transition-colors"><i class="fa-solid fa-graduation-cap"></i></a>
        <?php endif; ?>
        <?php if (!empty($m['lab_website'])): ?>
        <a href="<?= htmlspecialchars($m['lab_website']) ?>" target="_blank" rel="noopener" title="Website" class="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-rarl-red hover:text-white text-gray-500 dark:text-gray-300 flex items-center justify-center text-xs transition-colors"><i class="fa-solid fa-globe"></i></a>
        <?php endif; ?>
      </div>
    </div>
    <?php endforeach; ?>
